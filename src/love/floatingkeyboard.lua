--in its own lua file since its kinda big
local KEYSIZE = 72
--copied from flxkey
local ANY = -2;
local NONE = -1;
...
local keys = {
	--NONE = key(-1, 'none'),
	A = key(A, 'a', NONE, S, Q, NONE),
	B = key(B, 'b', V, N, H, SPACE),
	C = key(C, 'c', X, V, F, SPACE),
	D = key(D, 'd', S, F, E, X),
	...
	J = key(J, 'j', H, K, U, N),
	K = key(K, 'k', J, L, I, M),
	L = key(L, 'l', K, SEMICOLON, O, COMMA),
	M = key(M, 'm', N, COMMA, K, SPACE),
	N = key(N, 'n', B, M, J, SPACE),
	...
	V = key(V, 'v', C, B, G, SPACE),
	W = key(W, 'w', Q, E, TWO, S),
	X = key(X, 'x', Z, C, D, SPACE),
	Y = key(Y, 'y', T, U, SIX, H),
	Z = key(Z, 'z', SHIFT, X, S, SPACE),
	...
	SEMICOLON = key(SEMICOLON, ';', L, QUOTE, P, PERIOD, ':'),
	QUOTE = key(QUOTE, "'", SEMICOLON, ENTER, LBRACKET, SLASH, '"'),
	ENTER = key(ENTER, 'enter', QUOTE, NONE, RBRACKET, NONE),
	SHIFT = key(SHIFT, 'shift', NONE, Z, CAPSLOCK, CONTROL),
	COMMA = key(COMMA, ',', M, PERIOD, L, SPACE, '<'),
	PERIOD = key(PERIOD, '.', COMMA, SLASH, SEMICOLON, SPACE, '>'),
	SLASH = key(SLASH, '/', PERIOD, NONE, QUOTE, SPACE),
